...
export default function Home() {
  ...
  const filtered = useMemo(() => subjectTopics.filter((item) => {
    ...
    const text = `${item.title} ${item.group} ${item.summary} ${item.pages} ${item.sourceLabel}`.toLowerCase();
  ...
  function selectQuizOption(question, optionIndex) {
    ...
    if (!Array.isArray(question.answer)) {
      setAnswers((state) => ({ ...state, [question.key]: { selected: [optionIndex], locked: true } }));
      return;
    }
  ...
  function lockQuizQuestion(question) {
    const current = answers[question.key];
    if (!Array.isArray(question.answer) || !current?.selected?.length || current.locked) return;
  ...
  function selectSimulationOption(question, optionIndex) {
    ...
    if (current?.locked) return;
    if (!Array.isArray(question.answer)) {
      setSimAnswers((state) => ({ ...state, [question.key]: { selected: [optionIndex], locked: true } }));
      return;
    }
    const selectedOptions = new Set(current?.selected ?? []);
    if (selectedOptions.has(optionIndex)) selectedOptions.delete(optionIndex);
  ...
  return <main className="app-shell">
    <header className="topbar">
      <div className="brand">
        ...
        <button className="brand-mark" onClick={() => setView("beranda")} aria-label="Beranda PelajarinAja">PA</button>
    ...
    <aside className={`sidebar ${sidebar ? "open" : ""}`}>
      ...
      <div className="search-box"><Icon name="search" size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari materi..." /></div>
      ...
      <div className="topic-list">
        {filtered.map((item) => <button key={item.id} className={selected === item.id ? "active" : ""} onClick={() => chooseTopic(item.id, "materi")}><span className={`dot ${completed.includes(item.id) ? "done" : ""}`}>{completed.includes(item.id) ? "✓" : ""}</span><span><b>{item.title}</b><small>{item.group} · {item.pages}</small></span><Icon name="chevron" size={15} /></button>)}
      </div>
    </aside>
    ...
    <section className="content">
        ...
        <div className="question-stack">
          {topicQuestions.map((question, questionIndex) => {
            ...
            return <article className="card question-card" key={question.key}>
              <div className="question-meta"><div className="q-number">{multiple ? "Pilihan Ganda Kompleks" : "Pilihan Ganda"} {questionIndex + 1}</div><div className="meta-pills"><Pill>{question.difficulty}</Pill><Pill>{question.points} poin</Pill>{multiple && <Pill>Pilih semua yang benar</Pill>}</div></div>
              <h2>{question.q}</h2>
      ...
      {view === "progress" && <section className="progress-page">
        ...
        {bookmarks.length > 0 && <div className="card progress-card"><SectionTitle number="04" title="Materi tersimpan" subtitle="Bookmark lintas mata pelajaran."/><div className="bookmark-grid">{topics.filter((item) => bookmarks.includes(item.id)).map((item) => <button key={item.id} onClick={() => chooseTopic(item.id, "materi")}><Icon name="star"/><span><b>{item.title}</b><small>{subjects.find((subjectItem) => subjectItem.id === item.subjectId)?.name} · {item.pages}</small></span><Icon name="chevron"/></button>)}</div></div>}
      </section>}
    </section>
